...
if ($method === 'GET') {
    ...
    // Una imagen por ID (para el visor/AJAX)
    if (isset($_GET['id'])) {
        $id   = intval($_GET['id']);
...
if ($method === 'POST') {
    $esForm      = !empty($_POST);
    $titulo      = trim($esForm ? ($_POST['titulo']      ?? '') : (json_decode(file_get_contents('php://input'),true)['titulo']      ?? ''));
    $descripcion = trim($esForm ? ($_POST['descripcion'] ?? '') : (json_decode(file_get_contents('php://input'),true)['descripcion'] ?? ''));
    $orden       = intval($esForm ? ($_POST['orden']     ?? 0)  : (json_decode(file_get_contents('php://input'),true)['orden']      ?? 0));
    $activo      = intval($esForm ? ($_POST['activo']    ?? 1)  : (json_decode(file_get_contents('php://input'),true)['activo']     ?? 1));
    $url_ext     = trim($esForm ? ($_POST['url_imagen']  ?? '') : (json_decode(file_get_contents('php://input'),true)['url_imagen'] ?? ''));

    if (!$esForm) {
        $json = json_decode(file_get_contents('php://input'), true) ?? [];
        $titulo=$json['titulo']??''; $descripcion=$json['descripcion']??'';
        $orden=$json['orden']??0; $activo=$json['activo']??1; $url_ext=$json['url_imagen']??'';
    }
    if (empty($titulo)) { http_response_code(422); echo json_encode(['exito'=>false,'mensaje'=>'El título es requerido']); exit; }

    $url_imagen = '';
    if ($esForm && isset($_FILES['archivo']) && $_FILES['archivo']['error'] === UPLOAD_ERR_OK) {
        $url_imagen = subirArchivo($_FILES['archivo']);
        if (!$url_imagen) { echo json_encode(['exito'=>false,'mensaje'=>'Archivo no válido. Solo JPG/PNG/GIF/WEBP, máx 5 MB.']); exit; }
    } else { $url_imagen = $url_ext; }

    if (empty($url_imagen)) { http_response_code(422); echo json_encode(['exito'=>false,'mensaje'=>'Se requiere imagen (archivo local o URL)']); exit; }
    ...
    echo json_encode(['exito'=>true,'mensaje'=>'Imagen creada','id'=>$id,'url_imagen'=>$url_imagen]); exit;
}
...
if ($method === 'PUT') {
    ...
    echo json_encode(['exito'=>true,'mensaje'=>'Imagen actualizada','url_imagen'=>$url_imagen]); exit;
}
...
if ($method === 'DELETE') {
    ...
    echo json_encode(['exito'=>true,'mensaje'=>'Imagen eliminada']); exit;
}
